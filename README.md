...
is resumed by calling `run_workbook()` again. A sheet that cannot be
analysed does not stop the run: the reason is reported and the remaining
sheets are still fitted.
...
installation; `tinytex::install_tinytex()` provides one, or choose the
